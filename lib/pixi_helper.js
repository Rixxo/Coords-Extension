...
function resizeLine(line, x, y, x1, y1, options = new LineOptions()) {
    let lineLengthWithGap = lineLength + gap;
   
    let horizontal = isHorizontal(x,y,x1,y1);

    if (maxLineWidth + lineLengthWithGap < width && horizontal) {
        let numSegments = Math.floor((width- maxLineWidth)/lineLengthWithGap);
        maxLineWidth += numSegments*lineLengthWithGap;
        x1 = maxLineWidth;
    } else if (maxLineHeight + lineLengthWithGap < height && !horizontal) {
        let numSegments = Math.floor((height- maxLineHeight)/lineLengthWithGap);
        maxLineHeight += numSegments*lineLengthWithGap;
        y1 = maxLineHeight;
    } else return;
